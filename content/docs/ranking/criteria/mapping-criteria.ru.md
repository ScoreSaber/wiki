...
**Распределение сложностей (Difficulty Spread)**: набор уровней сложности, присутствующих в мапсете. Например, мапсет с тремя сложностями в виде Normal, Hard и Expert - имеет распределение из трёх уровней сложности.
...
Ритмический овермаппинг не допускается. Один звук не может быть представлен несколькими нотами, если только эти ноты не входят в один свинг.  
...
Аудио карты должно состоять из музыкальных элементов, организованных в распознаваемую структуру.  
...
- Названия сложностей не должны состоять исключительно из одного или нескольких имён пользователей или мапперов. Слова, которые одновременно являются никнеймами, допускаются, если они связаны с содержанием песни.
...
Паттерны, требующие ресета, должны предоставлять достаточное количество времени для выполнения ресета между соответствующими нотами, соразмерно сложности карты
...
Паттерны и секции карты не должны чрезмерно преувеличивать музыку, под которую они маппятся.  
...
Траектории предударных частей свинга пересекаются.  
